...
@keyframes led-banner-glow {
  ...
  <script>
    ...
    function getWeatherDescription(code) {
      const weatherCodes = {
        1000: "Trời quang", 1100: "Gần quang", 1101: "Có mây rải rác",
        1102: "Nhiều mây", 1001: "Mây dày", 2000: "Sương mù",
        2100: "Sương mù nhẹ", 4000: "Mưa phùn", 4001: "Mưa",
        4200: "Mưa nhẹ", 4201: "Mưa vừa", 4202: "Mưa to",
        8000: "Mưa rào"
      };
      return weatherCodes[code] || "Chưa xác định";
    }
    ...
    function getPrecipitationForecast(hourlyData) {
      ...
      if (!significantRain) return "Không có khả năng mưa đáng kể.";

      const time = new Date(significantRain.time);
      const probability = significantRain.values.precipitationProbability;
      return `Có khả năng mưa (${probability}%) vào khoảng ${time.getHours()}h.`;
    }
...
<button id="prev-btn" style="position:fixed;bottom:15px;right:80px;z-index:9999;">⏮️</button>
<button id="next-btn" style="position:fixed;bottom:15px;right:40px;z-index:9999;">⏭️</button>
<script>
  // Danh sách nhạc
  const tracks = [
  ...
  // Khi hết bài thì phát bài tiếp theo
